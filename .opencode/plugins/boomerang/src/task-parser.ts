import { Task, TaskWithAgent, DAG, ExecutionPlan, AgentType } from "./types.js";

export function parseTasksFromPrompt(prompt: string): Task[] {
...
export function detectImplicitDependencies(tasks: Task[]): Task[] {
  const updated = [...tasks];
...
export function buildDAG(tasks: TaskWithAgent[]): DAG {
...
export function createExecutionPlan(dag: DAG): ExecutionPlan {
  ...
  while (remaining.size > 0) {
    ...
    phases.push({
      phase: phaseNum++,
      type: (ready.length > 1 ? "parallel" : "sequential") as "parallel" | "sequential",
      tasks: ready,
    });
  }
...
export function formatDAGForPrompt(dag: DAG): string {
  let output = "```\n";
...
export function assignAgentsToTasks(tasks: Task[]): TaskWithAgent[] {
  return tasks.map((task) => {
    const lower = task.description.toLowerCase();
    let assigned: AgentType = "coder";
    for (const [agent, keywords] of Object.entries(AGENT_KEYWORDS)) {
      if (keywords.some((kw) => lower.includes(kw))) {
        assigned = agent as AgentType;
        break;
      }
    }
